...
class PacketHandler {
    public:
        ...
        bool handleSwapItem(HANDLE_ARGS);
        bool handleCastSpell(HANDLE_ARGS);
        bool handleTimerResponse(HANDLE_ARGS);
        ...
        bool sendPacket(ENetPeer *peer, SendablePacket &packet);
        bool sendToOthers(CHero *hero, uint8 *data, uint32 length, uint8 channelNo, uint32 flag = RELIABLE);
        bool broadcastPacket(uint8 *data, uint32 length, uint8 channelNo, uint32 flag = RELIABLE);
        bool broadcastPacket(SendablePacket &packet);

        void InitTowers(ENetPeer *peer);
        void ReleaseTowers();
    private:
