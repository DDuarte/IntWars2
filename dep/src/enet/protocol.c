...
enet_protocol_handle_incoming_commands(ENetHost *host, ENetEvent *event) {
    ...
    /*printf("[%i] [", host->receivedDataLength);
    {
        for(UINT i = 0; i < host->receivedDataLength; i++) {
            printf("%02X ", (BYTE)host->receivedData[i]);
        }
    }
    printf("]\r\n");*/
    ...
    { return 0; }
        ...
        if(peer -> state == ENET_PEER_STATE_DISCONNECTED ||
            ...
            /*char szMsg[124];
            sprintf(szMsg, "%i Error - %08X", peer->state, host->receivedAddress.host);
            printf("%s\r\n", szMsg);*/
            //MessageBox(0, szMsg, "omg why you hack", 0);
            return 0;
        }
        /*if (host -> checksum != NULL)
        {
...
enet_protocol_send_outgoing_commands(ENetHost *host, ENetEvent *event, int checkForTimeouts) {
    ...
    while(host -> continueSending)
        for(host -> continueSending = 0,
            ...
            else if(enet_list_empty(& currentPeer -> sentReliableCommands) &&
            ...
            header.peerID = 0x29; //byte peerId?
            header.flag = currentPeer->outgoingPeerID | host->headerFlags;
